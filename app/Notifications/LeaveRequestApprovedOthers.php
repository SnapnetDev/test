...
class LeaveRequestApprovedOthers extends Notification
{
    ...
    public function toDatabase($notifiable)
    {
        ...
        return new DatabaseMessage([
            ...
            'message'=>'The leave request, '.$leave_name.' which was submitted by '.$this->leave_approval->leave_request->user->name.' for approval on the '.date('Y-m-d',strtotime($this->stage->workflow->name)).'('.\Carbon\Carbon::parse($this->leave_approval->leave_request->created_at)->diffForHumans().') has been approved at the final stage.<br> Their leave balance is '.$this->leave_approval->leave_request->balance.'
